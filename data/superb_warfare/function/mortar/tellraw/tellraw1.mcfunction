...
#顯示使用者輸入射擊參數
    ...
    #砲擊座標
        ...
        #自己作為座標[非OP]
        execute as @e[tag=mortar_parameter] if score @s mortar_parameter matches 1 run tellraw @s [{"text":"[非OP]","color":"red","hoverEvent":{"action":"show_text", "value":[{"text":"不需要OP權限"}]}},{"text":"點擊此迫擊砲將以你自己座標作為砲擊座標","color":"green","clickEvent":{"action":"run_command","value":"/trigger mortar_no_op set 1"},"hoverEvent":{"action":"show_text", "value":[{"text":"記得找好掩護","italic":true}]}}]
