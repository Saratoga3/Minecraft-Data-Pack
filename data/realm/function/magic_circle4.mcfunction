#半徑10格的圓
    ...
    #產生粒子特效
    execute rotated as @e[tag=a] positioned ^ ^ ^10 run particle flame ~ ~0.1 ~ 0 0 0 0.1 0 force

    #記分板 & 停止條件
    scoreboard players add t time 1
    execute if score t time matches ..50 run function realm:magic_circle4
    execute if score t time matches 51 run scoreboard players set t time 0

#execute at @e[tag=missile] as @e[name="Li___Bai"] run tp ~10 ~ ~